...
Applied by the `excel-pipeline-networking-prod` workspace. Creates the VPC
and subnet that every other prod-environment domain (`gke`, `data`,
`cloud-run`) runs inside.

## What this creates

- The `excel-pipeline-vpc-prod` VPC network, in custom subnet mode (no
  auto-created subnets).
- The `gke-subnet-prod-europe-west9` regional subnet, with Private Google
...
- Two secondary IP ranges on that subnet, reserved for GKE Autopilot's
  alias IPs — `gke-pods-prod` and `gke-services-prod`. Reserved now, before
...
|---|---|---|---|---|
| `gke-subnet-prod-europe-west9` | Primary | `10.16.0.0/20` | 4,096 | GKE node IPs |
| `gke-pods-prod` | Secondary | `10.20.0.0/20` | 4,096 | Pod alias IPs (GKE Autopilot IP aliasing) — real, routable within the VPC |
| `gke-services-prod` | Secondary | `10.24.0.0/20` | 4,096 | Service (`ClusterIP`) addresses — virtual, translated via iptables/eBPF on each node, never routed on the VPC |
...
| `TFC_GCP_PROVIDER_AUTH` | Environment variable | Inherited from the `prod-credentials` variable set — no manual action needed |
...
needs to be attached to the `prod` HCP Terraform Project, and it inherits
